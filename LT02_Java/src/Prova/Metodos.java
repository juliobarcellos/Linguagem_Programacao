...
public class Metodos {
	...
	Livros[] FCadastraGrava(Livros[] livros) throws IOException {
		...
		for (Cont = 0; Cont < 10; Cont++) {
			...
			grava.newLine();
			livros[Cont].QtdL = Integer
					.parseInt(JOptionPane.showInputDialog("Digite a quantidade de vezes que o livro foi emprestado"));
	...
	void PMostraQtd(Livros[] livros) {
		...
		for (Cont1 = 0; Cont1 < 10; Cont1++) {
			if (livros[Cont1].QtdL == livros[0].QtdL) {
				Dados += ("Código:" + livros[Cont1].CodL + " Qtde:" + livros[Cont1].QtdL + "\n");
			} else {
				break;
			}
		}
