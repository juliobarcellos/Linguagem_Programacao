...
public class Caixa_Eletronico {
		...
		for (int i = 0; i < _notas.length; i++) {
			if (_saque >= cedulas[i]) {
				do {
					...
					n = sc.nextInt();
					if (n > notas[i]) {
						System.out.printf("\nNao ha notas sufucientes, quantidade de notas no caixa %d: ", notas[i]);
					} else if (n * cedulas[i] > _saque) {
						System.out.printf("\nQuantidade de cedulas maior que o valor do saque\n");
					}
	...
	public static void RetirarNotas() {
		...
		if (valor > valorCaixa) {
			...
			do {
				System.out.printf("\n1 - %s\n2 - %s\n3 - %s\n4 - %s\nDigite o codigo do banco: ", nomeBancos[0],
			...
			do {
				...
				if (dec.equals("S") && valor >= 20) {
					...
					System.out.printf("\nPara poder escolher a quantidade de notas, voce precisa retirar mais de 20 reais\n");
	...
	public static int CarregarNotas() {
		...
		if (valor == 1) {
			_valorCaixa = 0;
			System.out.printf("\nNao e possivel depositar esse valor\n");
		} else {
			for (int i = 0; i < _notas.length; i++) {
				notas[i] += _notas[i];
			}
