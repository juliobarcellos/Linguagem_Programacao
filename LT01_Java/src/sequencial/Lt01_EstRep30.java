...
public class Lt01_EstRep30 {
	public static void main(String args[]) {
		int AnoN = 0, AnoA = 0, MesN = 0, MesA = 0, DiaN = 0, DiaA = 0, NDiaN = 0, NDiaA = 0, AnoT = 0, MesT = 0,
				DiaT = 0;
		String Idade = "Idade: ";
		Calendar cal = Calendar.getInstance();

		AnoA = cal.get(Calendar.YEAR);
		MesA = (cal.get(Calendar.MONTH) + 1);
		...
		do {
			...
			if (AnoN == AnoA && MesN > MesA) {
				JOptionPane.showMessageDialog(null,
						"Valor incorreto, Se o ano de nascimento é igual ao ano atual, o mês de nascimento não pode ser maior que o atual");
			}
			if (MesN > 12 || MesN < 1) {
		...
		do {
			...
			if (AnoA == AnoN && MesN == MesA && DiaN > DiaA) {
				JOptionPane.showMessageDialog(null,
						"Valor incorreto, Se o ano e mês de nascimento são iguais ao ano e mês atual, o dia de nascimento não pode ser maior que o atual");
			}
